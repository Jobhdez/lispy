  ...
  let tempName = "temp_" ++ show counter in
    let tempName2 = "temp_" ++ show (counter + 1) in
      let tempName3 = "temp_" ++ show (counter + 2) in
        case cnd of
          (If (Bool b) thn3 els3) -> CLet [(tempName, CIf (ABool b) (toanf thn3 (counter+2)) (toanf els3 (counter+2)))] (CLet [(tempName2, (CIf (AVar tempName) (toanf thn (counter+3)) (toanf els (counter+3))))] (toanf (If (Var tempName2) thn2 els2) (counter + 4)))
          (If (Less a b) thn3 els3) -> CLet [(tempName, (tocomplex (Less a b)))] (CLet [(tempName2, (CIf (AVar tempName) (toanf thn3 (counter + 2)) (toanf els3 (counter+2))))]  (CLet [(tempName3, (CIf (AVar tempName2) (toanf thn (counter + 3)) (toanf els (counter + 3))))] (toanf (If (Var tempName3) thn2 els2) (counter + 4))))
